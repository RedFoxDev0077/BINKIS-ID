...
**Claim Code** — 11 chars displayed `XXXX-XXXX-XXX`, e.g. `7K9P-2M4X-Q3F`. Ten random from the same 31-char alphabet (0/O/1/I/L excluded so it can't be misread off foil), plus one check character (weighted checksum over GF(31): `check = -Σ wᵢ·xᵢ mod 31`, weights 2..11 on the payload and 1 on the check character itself). The check character is validated client-side so a typo never reaches the database and never burns a rate-limited attempt. Because 31 is prime and every weight is distinct and non-zero, this detects **every** single-character substitution and **every** transposition, adjacent or not. Roughly 1 in 6 billion per guess against ~140k live codes.
